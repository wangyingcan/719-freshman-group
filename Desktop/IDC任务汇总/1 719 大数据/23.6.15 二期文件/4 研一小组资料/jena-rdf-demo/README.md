...
2. **读写方法**的参照是  `文件`
   ...
   > - **==程序产生的内容   输出到    文件==**：利用输出流包装文件，然后利用写方法写入
   > - **==文件   输入到   程序==**：利用输入流包装文件，然后利用读方法读取
   >
   > 输入输出不仅限于文件IO流，也可以是标准输入输出流(System.out、System.in)
...
# 719-freshman-group
719项目组新研一的资料仓库
>>>>>>> f1e230fc813abf2d4c31aeac3a5ecd213dd39906
